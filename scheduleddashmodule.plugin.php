...
class ScheduledDashModule extends Plugin
{
	...
	/**
	 * Produce the content for the latest scheduleds block
	 * @param Block $block The block object
	 * @param Theme $theme The theme that the block will be output with
	 */
	public function action_block_content_scheduled_posts($block, $theme)
	{
		$block->recent_posts = Posts::get(array('status' => 'scheduled', 'limit' => 8, 'user_id' => User::identify()->id));
		if(User::identify()->can('manage_entries'))
		{
			$block->link = URL::get('admin', array('page' => 'posts', 'status' => Post::status('scheduled'), 'user_id' => User::identify()->id));
		}
	}
